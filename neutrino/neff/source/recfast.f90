...
!    Atomic data for HeI 
        ...
        !real(8)    :: grhom, grhog, grhon, grhoc, grhob, grhov
        ...
        public RecombinationParams, Recombination_xe, Recombination_tm,Recombination_ts ,Recombination_init,   &
               ...
               recombination_saha_tau, recombination_saha_z, RECFAST_Heswitch_default, &
               RECFAST_fudge_He_default, RECFAST_Hswitch_default, &
       ...
       contains
        ...
        subroutine Recombination_init(Recomb, OmegaC, OmegaB, Omegan, Omegav, h0inp,tcmb,yp,neff)
        ...
        real(dl), save :: last_OmB =0, Last_YHe=0, Last_H0=0, Last_detada=0, last_fudge, last_fudgeHe 
...
!       ===============================================================
        ...
        !grhom = 3*h0inp**2/c**2*1000**2
		!grhog = kappa/c**2*4*sigma_boltz/c**3*Tcmb**4*Mpc**2
		!grhon = Neff * 7.d0/8.d0*(4.d0/11.d0)**(4.d0/3.d0)*grhog
		!grhoc = grhom*OmegaC
        !grhob = grhom*OmegaB
        !grhov = grhom*Omegav

        if (Last_OmB==OmegaB .and. Last_H0 == h0inp .and. yp == Last_YHe .and. & 
             deta_da(0.2352375823_dl) == Last_detada .and. last_fudge == Recomb%RECFAST_fudge &
        ...
        Last_detada =  deta_da(0.2352375823_dl) !Just get it at a random scale factor
        Last_OmB = OmegaB
        Last_H0 = h0inp
        Last_YHe=yp
...
!       sort out the helium abundance parameters
        ...
        n = Nnow * (1._dl+z)**3
        fnu = neff*(7.d0/8.d0)*(4.d0/11.d0)**(4.d0/3.d0)
...
! He is 99% singly ionized, and *then* switch to joint H/He recombination.
          ...
          if (doTmatTspin) then
              if (Evolve_Ts .and. zend< 1/Do21cm_minev-1 ) then
               ...
               tau_21Ts = line21_const*NNow*(1+zend)*deta_da(1/(1+zend))/1000
...
!        Tspin = y(4)
        ...
        Hz = 1/deta_da(1/(1._dl+z))*(1._dl+z)**2/MPC_in_sec       
...
!       the Pequignot, Petitjean & Boisson fitting parameters for Hydrogen    
        ...
        n_He = fHe * n
        Trad = Tnow /a
        clh = 1/deta_da(a)/a !conformal time
...
!          kappa_HH_21cm = exp(polevl(logT,fit,7))*0.01**3          
        ...
        !    a2 = a**2.d0
        !    grhoa2 = (grhoc+grhob)*a + grhog + grhon
        !    grhoa2 = grhoa2 + grhov*a2**2.d0

        !    dtauda = sqrt(3.d0/grhoa2)
        !
        !    return
        !end function dtauda
